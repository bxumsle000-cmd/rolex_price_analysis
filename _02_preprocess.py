...
class DataPreprocessor:
    ...
    def convert_price_to_int(self):
        """轉換價格欄位為整數"""
        price_cols = ["price", "aditional shipping price"]
    ...
    def encode_categorical(self, columns= ['movement', 'condition', 'material_group', 'country']):
        """
...
if __name__ == "__main__":
    ...
    # # 方法 2: 逐步執行（更靈活）
    preprocessor = DataPreprocessor("data/data.csv")
    preprocessor.load_data() # 載入資料
    preprocessor.remove_outliers(iqr_multiplier=1.5)  # 移除價格異常值 (by ref number)
    preprocessor.impute_all()  # (age、case_diameter、movement、material_group、condition、convert_price_to_int)
    preprocessor.encode_categorical(['movement', 'condition', 'material_group', 'country']) # 對類別資料進行編碼
    df_clean = preprocessor.get_data() # 取得清理過後的資料
